...
async def run_scraper(keyword, location):
    ...
    async with async_playwright() as p:
        ...
        while True:
            last_count = len(page.locator('div[role="main"] a[href*="!1s"]').all_text_contents())
            ...
            new_count = len(page.locator('div[role="main"] a[href*="!1s"]').all_text_contents())
        ...
        for card in business_cards:
            ...
            new_lead_df = pd.DataFrame([business_data])
...
# Display initial empty dataframe or previous results
data_table_placeholder.dataframe(st.session_state.leads_df, use_container_width=True)
log_placeholder.text_area("Live Status Console", value="\n".join(st.session_state.log_messages[::-1]), height=300, key="log_area_initial")

# --- Potential Enhancements (Manus AI suggestions) --- #
# 1. Asynchronous Execution for Streamlit: For long-running tasks like scraping, consider using a separate process or a library like `streamlit-extras.st_async`
#    to prevent the Streamlit UI from freezing. The current implementation uses `asyncio.run` which blocks the main thread.
# 2. Proxy Integration: For block resistance, integrate a proxy rotation service.
...
# 3. CAPTCHA Solving: If CAPTCHAs are encountered, integrate a CAPTCHA solving service API (e.g., 2Captcha, Anti-Captcha).
#    This would involve detecting CAPTCHAs and sending them to the service for resolution.
# 4. Advanced Email Hunting: For more comprehensive email extraction, consider using dedicated email finder APIs
...
# 8. Error Handling & Retries: Implement more sophisticated retry logic with exponential backoff for network requests and scraping failures.
